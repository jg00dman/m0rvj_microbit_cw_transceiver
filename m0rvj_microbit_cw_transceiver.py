...
#   A shake opens a menu and you make selections by sending a character.
#   Settings are retained.
#   Connect Speaker or Headphones to Pin 0 and Ground
#   Or connect Piezo Buzzer or Haptic Motor for Silent operation pin1
...
radio.on()
radio.reset()
conf = ['A', 'H', 'N', 'U']  # "ABCDEF"=speed GHIJKLM=tone NOPQRS=channel T=tutor UV=power"
di = 80
dah = 240
space = 560
tx = True
...
#        display.scroll('-')
    ...
    for i in conf:
        x = ord(i) - 64
        if x < 1 or x > 23:  #out of range / unknown command
            playMorse(enc('?'))
            return
            ...
            conf[3] = i
            power = 'QRP'
        elif x is 23:  # W=reset
            conf = ['A', 'H', 'N', 'U']
        else: pass  #xyz unknown command
    if command:
        conf = conf[:4]
        if command[0] is 'T':
